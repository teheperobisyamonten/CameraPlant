...
- Measureツールでの縮尺（再）校正：Measureツールで部屋の壁や既知のサイズの物体などに
  線を引き、実距離を入力して「Set Scale」を押すと、その計測を基準にシステム全体の
  縮尺を再計算する。Camera/Subjectの距離・FOV・被写界深度・パーソナルスペースなど、
  縮尺を参照している表示すべてに即座に反映される（「Set Scale」ボタンは初回セット
  アップ用として引き続き利用可能。両者は併用できる）
- 縮尺（1m＝何px）はサイドバーの数値入力欄で直接編集・修正することも可能
  （Measureツールや2点クリックで校正した後の微調整に使える。0以下やNaNなどの
  不正な値は拒否される）
- 縮尺を変更すると、Camera・Subjectのアイコンの大きさも実寸に応じて自動的に
  拡大・縮小される（未設定時は従来通りの固定サイズで表示）

### カメラ・レンズ
- カメラの配置・選択・ドラッグ移動・回転・削除
- Objects欄のカメラ／被写体アイコンを平面図上へドラッグ＆ドロップして配置可能
  （アイコンをクリックするだけでも中央付近に配置できる、従来通りの操作も引き続き利用可能）
...
本リポジトリはPublicで、`gh-pages` ブランチの静的ビルド成果物を GitHub Pages で
公開しています（GitHub Pagesは現在のプランでは非公開リポジトリからは公開できない
ため、Web公開にはリポジトリ自体をPublicにする必要があります）。コードを更新した
場合の再公開手順：
